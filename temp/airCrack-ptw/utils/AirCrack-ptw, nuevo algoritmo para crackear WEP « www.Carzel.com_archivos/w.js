...
if ( _error ) {
	var c = escape(_error);
	url=pr+"//stats.wordpress.com/e.gif?c="+c+"&u="+escape(document.location)+"&rand="+Math.random();
} else if ( _admin ) {
	var p = escape(_page);
...
function linkclick(event) {
	var isIE=window.attachEvent?true:false;
	event=event?event:(window.event?window.event:"");
	if (_wpc.modo==false&&_wpc.hold==false) {
		_wpc.hold = true;
		linktracker_record(event);
	}
}
function linktracker_init(b,p){
	...
	window.onunload = function(){while(_wpc.i.complete==false&&_wpc.nada<1000){++_wpc.nada;};return true;};
}
function linktracker_record(event){
	...
	var r=(typeof b.rel != 'undefined')?escape(b.rel):'0';
	var b=(typeof _blog != 'undefined')?_blog:0;
	var p=(typeof _post != 'undefined')?_post:0;
	_wpc.i.src=pr+'//stats.wordpress.com/c.gif?b='+b+'&p='+p+'&r='+r+'&u='+escape(bh)+"&rand="+Math.random();
	_wpc.i.onLoad=function(){cmcVoid();}
}
var _wpc={modo:false,hold:false,nada:0,i:new Image(1,1)};
