...
f'isicos, y para un tipo espec'ifico, como un disco duro, existen
diferentes interfaces de manejo entre un fabricante de hardware y
otro. Adem'as, Linux posee soporte para distintos sistemas de
archivos l'ogicos que lo hace interoperante entre distintos sistemas
operativos, con los siguientes prop'ositos:
...
para llamadas de usuarios y otra para los subsistemas del kernel. La
primera maneja archivos y directorios. Las operaciones en archivos
incluyen $open, read, close, write, seek$, que est'an definidas en
el estandar $POSIX$ y en los directorios $readdir, creat, unlink,
chmod, stat$ tambi'en definidas en $POSIX$. La otra es bastante m'as
interesante: posee estructuras de datos y funciones para
...
posee tres tipos de ellos: $char$, $block$ y $network$. Los dos m'as
relevantes son $char$ y $block$. Los primeros son aquellos cuya
lectura es secuencial (de a un char a la vez), como un mouse. Los
segundos pueden ser accedidos de cualquier manera, pero solo puede
leerse o escribirse de a bloques.

Todos los drivers soportan las operaciones reci'en mencionadas.
...
kernel busca el driver correcto para invocar. Cuando una
interrupci'on es tomada, la CPU se ejecuta en un contexto especial,
es decir, todas las interrupciones quedan pendientes hasta que la
interrupci'on actual haya terminado. Debido a esto, los manejadores
de interrupciones deben ser altamente eficientes. Si ocurre el caso
que un manejador de interrupciones no puede terminar el trabajo,
reasigna el trabajo pendiente en un manejador de tipo
``bottom\-half'', osea, c'odigo que se ejecutar'a la pr'oxima vez
que la llamada sea completada para evitar latencias y promover la
concurrencia.
...
virtual(VFS). Esto significa que un bloque asociado a un dispositivo
...
Para el soporte de filesystems virtuales, Linux usa \textbf{Inodos}
para representar un archivo en un dispositivo $block$. El inodo es
virtual en el sentido que contiene operaciones que est'an
implementadas de distintas maneras, dependiendo del sistema l'ogico
y f'isico donde el archivo reside. El inodo es usado como un lugar
de almacenamiento para toda la informaci'on relacionada con el
manejo de, por ejemplo, abrir un archivo del disco. Guarda buffers,
el largo del archivo , etc.
...
El caso ideal es en el que el dispositivo pudiera ocuparse y
